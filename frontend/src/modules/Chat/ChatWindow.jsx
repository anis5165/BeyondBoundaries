...
const ChatWindow = ({ receiverId, receiverName, onClose }) => {
    ...
    useEffect(() => {
        if (currentMember && receiverId) {
            loadMessages();
        }
    }, [currentMember, receiverId]);

    const loadMessages = async () => {
        try {
            const response = await axios.get(`http://localhost:5000/message/chat/${currentMember._id}/${receiverId}`);
    ...
    const sendMessage = async (e) => {
        ...
        try {
            ...
            const response = await axios.post('http://localhost:5000/message/add', messageData);
